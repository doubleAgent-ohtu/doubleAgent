...
def test_client():
	...
	main.app.dependency_overrides[main.get_current_user] = (
		lambda: {"sub": "test-user", "username": "tester"}
	)
...
def test_chat_endpoint_with_a_and_b(test_client, monkeypatch):
	...
	monkeypatch.setattr(
		main.chatbot_a, "chat", lambda message, thread_id, system_prompt, model: "AI-A: pong"
	)
	monkeypatch.setattr(
		main.chatbot_b, "chat", lambda message, thread_id, system_prompt, model: "AI-B: pong"
	)
...
def test_download_chat(test_client, monkeypatch):
	...
	monkeypatch.setattr(main.chatbot_a, "get_conversation_history", lambda thread_id: "Bot A:\nHello\n")

	r = test_client.get("/download-chat/t123")
	assert r.status_code == 200
	assert r.headers.get("content-disposition") == "attachment; filename=conversation_t123.txt"
